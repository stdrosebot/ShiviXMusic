...
from ShiviX.misc import HAPP, SUDOERS, XCB
from ShiviX.utils.database import (get_active_chats,
                                       remove_active_chat,
                                       remove_active_video_chat)
from ShiviX.utils.decorators.language import language
from ShiviX.utils.pastebin import Shivibin
...
<u>remaining dynos:</u>
Tota left: `{hours}`**H**  `{minutes}`**M**  [`{percentage}`**%**]"""
    ...
    if await is_heroku():
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"{config.MUSIC_BOT_NAME} has just restarted herself for updating the bot. sorry for the ishues.\n\nyou can start playing again after 15-20 seconds.",
                    )
                    ...
                    pass
            await response.edit(
                f"{nrs.text}\n\nbot updates successfully ! now wait for few minutes until the bot restarts and push changes !"
            )
            os.system(
        ...
        for x in served_chats:
            try:
                await app.send_message(
                    x,
                    f"{config.MUSIC_BOT_NAME} has just restarted herself for updating the bot. sorry for the issues.\n\nyou start playing again after 15-20 seconds.",
                )
                await remove_active_chat(x)
                await remove_active_video_chat(x)
            except Exception:
                pass
        await response.edit(
            f"{nrs.text}\n\nBot updated successfullyn ! now wait for few minutes until the bot restarts and push changes !"
        )
