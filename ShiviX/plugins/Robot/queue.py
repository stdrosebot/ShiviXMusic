...
from ShiviX.utils import (Shivibin, get_channeplayCB,
                              seconds_to_min)
from ShiviX.utils.database import (get_cmode, is_active_chat,
                                       is_music_playing)
from ShiviX.utils.decorators.language import language, languageCB
from ShiviX.utils.inline import queue_back_markup, queue_markup
...
async def queue_back(client, CallbackQuery: CallbackQuery, _):
    ...
    send = (
        ...
        if DUR == "Unknown"
        else "\nclick on button to get whole queued list."
    )
    cap = f"""**{config.MUSIC_BOT_NAME} player**
