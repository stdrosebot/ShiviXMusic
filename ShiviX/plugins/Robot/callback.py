...
from ShiviX import YouTube, app
from ShiviX.core.call import Shivi
from ShiviX.misc import SUDOERS, db
from ShiviX.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from ShiviX.utils.decorators.language import languageCB
from ShiviX.utils.formatters import seconds_to_min
from ShiviX.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from ShiviX.utils.stream.autoclear import auto_clean
from ShiviX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                await Shivi.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Shivi.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
